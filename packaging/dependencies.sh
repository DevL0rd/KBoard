...
if image_based_system; then
    source "$(CDPATH='' cd -- "$(dirname -- "$0")" && pwd)/build-box.sh"
    prepare_build_box
    exit 0
fi
...
*)
    echo "Unsupported package manager: install the Qt 6, KDE Frameworks 6, SDL3 and Hunspell development packages, then run ./install.sh --skip-deps"
    exit 1
    ;;
esac
