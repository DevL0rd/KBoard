...
mkdir -p "$out"
find /usr /etc /var/lib -xdev -printf '%p %y\n' | grep -vE '^/var/lib/(systemd|pacman|lastlog)/' | sort >"$out/system"
find /etc -xdev -type f ! -name machine-id -exec sha256sum {} + 2>/dev/null | sort -k2 >"$out/etc"
cd /home/tester
find . -xdev \( -path ./KBoard -o -path ./.cache \) -prune -o -type f -print0 | xargs -0 sha256sum | sort -k2 >"$out/home-files"
find . -xdev \( -path ./KBoard -o -path ./.cache \) -prune -o -type d -print | sort >"$out/home-folders"
